...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                    ...
                    <listBibl>
                        <biblFull>
                            ...
                            <titleStmt>
                                <xsl:choose>
                                    <xsl:when
                                        test="tei:text/tei:body/tei:listBibl/tei:biblFull/tei:titleStmt/tei:title">
                                        <xsl:for-each
                                            select="tei:text/tei:body/tei:listBibl/tei:biblFull/tei:titleStmt/tei:title">
                                            <title>
                                                <xsl:copy-of select="@*"/>
                                    ...
                                    <xsl:when
                                        test="tei:text/tei:body/tei:listBibl/tei:biblFull/tei:sourceDesc/tei:biblStruct/tei:analytic/tei:title">
                                        <xsl:for-each
                                            select="tei:text/tei:body/tei:listBibl/tei:biblFull/tei:sourceDesc/tei:biblStruct/tei:analytic/tei:title">
                                            <title>
                                                <xsl:copy-of select="@*"/>
                                                <xsl:value-of select="normalize-space(.)"/>
                                            </title>
                                    ...
                                    <xsl:when
                                        test="tei:text/tei:body/tei:listBibl/tei:biblFull/tei:sourceDesc/tei:biblStruct/tei:monogr/tei:title">
                                        <xsl:for-each
                                            select="tei:text/tei:body/tei:listBibl/tei:biblFull/tei:sourceDesc/tei:biblStruct/tei:monogr/tei:title">
                                            <title>
                                                <xsl:copy-of select="@*"/>
                                                <xsl:value-of select="normalize-space(.)"/>
                                            </title>
                                ...
                                <!-- funders -->
                                <xsl:apply-templates
                                    select="tei:text/tei:back//tei:listOrg[@type = 'projects']"/>
                            </titleStmt>
                            ...
                            <editionStmt>
                                ...
                                <respStmt>
                                    <xsl:variable name="halEdition" select="tei:text/tei:body/tei:listBibl/tei:biblFull/tei:editionStmt/tei:edition"/>
                                    <resp>Generated by TEI-Conditor XSLT (https://github.com/conditor/tei-conditor), from original HAL document <xsl:value-of select="//tei:idno[@type='halId']"/> (version &quot;<xsl:value-of select="$halEdition/@n"/>&quot;, type &quot;<xsl:value-of select="$halEdition/@type"/>&quot;)</resp>
                                    <name>Conditor</name>
                                </respStmt>
                            </editionStmt>
                            ...
                            <xsl:if
                                ...
                                <notesStmt>
                                    <xsl:apply-templates
                                        select="tei:text/tei:body/tei:listBibl/tei:biblFull/tei:notesStmt"
                                    />
                                </notesStmt>
                            </xsl:if>
                            ...
                            <sourceDesc>
                                <biblStruct>
                                    <xsl:apply-templates
                                        select="tei:text/tei:body/tei:listBibl/tei:biblFull/tei:sourceDesc/tei:biblStruct"/>
                                    <!-- crée :
    ...
    <xsl:template match="tei:notesStmt">
        <xsl:for-each select="tei:note">
            <note xmlns="http://www.tei-c.org/ns/1.0">
                ...
                <xsl:copy-of select="@n"/>
                <xsl:choose>
                    <xsl:when test="@type = 'audience'">
                        <!-- 0=not set, 1=international, 2=national, 3 idem -->
                    ...
                    <xsl:when test="@type = 'invited'">
                        <!-- 0=no, 1=yes  -->
                        <!-- mandatory metadata invitedCommunication ; mais absente des notices récentes exemple -->
                        <xsl:if test="@n = '0'">invited communication : no</xsl:if>
                        <xsl:if test="@n = '1'">invited communication : yes</xsl:if>
                    </xsl:when>
                    <xsl:when test="@type = 'popular'">
                        <!-- 0=not set, 1=international, 2=national -->
                    ...
                    <xsl:when test="@type = 'thesisOriginal'">
                        <!-- 0=not set, 1=international, 2=national -->
            ...
            <org xmlns="http://www.tei-c.org/ns/1.0">
                ...
                <xsl:if test="key('orgsById', $aff)/tei:orgName[@type = 'acronym']">
                    <orgName type="acronym">
                        <xsl:value-of
                            select="normalize-space(key('orgsById', $aff)/tei:orgName[@type = 'acronym'])"
                        />
                    </orgName>
                </xsl:if>
                ...
                <xsl:for-each select="/tei:TEI//tei:org[@xml:id = $aff]">
                    <!-- for-each pour descendre au niveau idno, traité par le call-template avec for-each -->
                    <xsl:call-template name="IDNO"/>
                </xsl:for-each>
    ...
    <xsl:template name="Affiliation">
        ...
        <xsl:for-each select="ancestor::tei:TEI//tei:org[@xml:id = $RelAct]">
            ...
            <!-- élimine niveaux intermédiaires pour certains, demande A. Coret et D Le Hennaff, mais dommage -->
            <org xmlns="http://www.tei-c.org/ns/1.0">
                <xsl:attribute name="type">
                    <xsl:value-of select="@type"/>
            ...
            <!-- désactivation de la récursivité, qui génère plein de doublons dans les affiliations -->
...
-->
    ...
    <xsl:template match="tei:biblStruct">
        ...
        <idno type="halId" xmlns="http://www.tei-c.org/ns/1.0">
        ...
        <monogr xmlns="http://www.tei-c.org/ns/1.0">
            <xsl:for-each select="tei:idno">
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <!-- corrects dans la source : issn, eissn, isbn notamment -->
            ...
            <xsl:if test="tei:authority[@type = 'supervisor']">
                <editor role="ths">
                    ...
                    <xsl:if test="tei:authority[@type = 'supervisor']/tei:idno">
                        <xsl:call-template name="IDNO"/>
                    </xsl:if>
                </editor>
            </xsl:if>
            <xsl:if test="tei:authority[@type = 'institution']">
                <editor role="dgg">
                    ...
                    <xsl:if test="tei:authority[@type = 'institution']/tei:idno">
                        <xsl:call-template name="IDNO"/>
                    </xsl:if>
                </editor>
            </xsl:if>
            <xsl:if test="tei:authority[@type = 'school']">
                <editor role="dos">
                    <orgName>
                        <xsl:value-of select="tei:authority[@type = 'school']"/>
                        <!--  @type=institution corrigé en "school"  -->
                    </orgName>
                    <xsl:if test="tei:authority[@type = 'school']/tei:idno">
                        <xsl:call-template name="IDNO"/>
                    </xsl:if>
                </editor>
            </xsl:if>
            <xsl:if test="tei:editor">
                <editor role="edt">
                    ...
                    <xsl:if test="tei:editor/tei:idno">
                        <xsl:call-template name="IDNO"/>
                    </xsl:if>
                    <!-- CS : récupère aussi l'ISBN, pas uniquement les id auteur  cf. hal-01442304  -->
                </editor>
            </xsl:if>
            ...
            <xsl:if test="tei:respStmt">
                <!-- organisateur meeting, séparé dans HAL -->
            ...
            <xsl:if test="tei:country">
                <!-- sens à voir - pour cartes, brevets, img, cours - idem country -->
        ...
        <xsl:for-each select="tei:idno">
            ...
            <xsl:choose>
                ...
                <xsl:when test="@type = 'halauthorid'">
                    <idno type="halAuthorId" xmlns="http://www.tei-c.org/ns/1.0">
                        <xsl:value-of select="."/>
                    </idno>
                </xsl:when>
                ...
                <xsl:when test="@type = 'ORCID'">
                    <idno type="orcid" xmlns="http://www.tei-c.org/ns/1.0">
                        <xsl:value-of select="."/>
                    </idno>
                </xsl:when>
                <xsl:when test="@type = 'VIAF'">
                    <idno type="viaf" xmlns="http://www.tei-c.org/ns/1.0">
                        <xsl:value-of select="."/>
                    </idno>
                </xsl:when>
                <xsl:when test="@type = 'ISNI'">
                    <idno type="isni" xmlns="http://www.tei-c.org/ns/1.0">
                        <xsl:value-of select="."/>
                    </idno>
                </xsl:when>

                <xsl:when test="@type = '1'">
                    <idno type="arxiv" xmlns="http://www.tei-c.org/ns/1.0">
                        <xsl:value-of select="."/>
                    </idno>
                </xsl:when>
                ...
                <xsl:when test="@type = '2'">
                    <idno type="researcherId" xmlns="http://www.tei-c.org/ns/1.0">
                        <xsl:value-of select="."/>
                    </idno>
                </xsl:when>
                ...
                <!-- copy-of ne remet pas xmlns hal -->
                <xsl:when test="@type = 'RNSR'">
                    <idno type="rnsr" xmlns="http://www.tei-c.org/ns/1.0">
                        <xsl:value-of select="."/>
                    </idno>
                </xsl:when>
                <xsl:when test="@type = 'IdUnivLorraine'">
                    <idno type="IdUnivLorraine" xmlns="http://www.tei-c.org/ns/1.0">
                        <xsl:value-of select="."/>
                    </idno>
                </xsl:when>
                ...
                <xsl:when test="@type = 'artNumber'">
                    <idno type="artNo" xmlns="http://www.tei-c.org/ns/1.0">
                        <xsl:value-of select="."/>
                    </idno>
                </xsl:when>

                <xsl:when test="@type = 'doi'">
                    <idno type="doi" xmlns="http://www.tei-c.org/ns/1.0">
                        <xsl:value-of select="."/>
                    </idno>
                </xsl:when>
                <xsl:when test="@type = 'DOI'">
                ...
                <xsl:when test="@type = 'programme'">
                    <idno type="programme" xmlns="http://www.tei-c.org/ns/1.0">
                        <xsl:value-of select="."/>
                    </idno>
                </xsl:when>
